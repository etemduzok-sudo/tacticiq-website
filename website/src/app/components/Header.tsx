...
export function Header({ onNavigate }: HeaderProps) {
  ...
  const handleNavClick = (key: string, href: string) => {
    // Scroll to section - sadece kullanıcı tıkladığında, otomatik değil
    if (href === '#') {
      // Ana sayfa için scroll yapma - kullanıcı zaten orada
      // window.scrollTo({ top: 0, behavior: 'smooth' }); // Kaldırıldı
    } else {
