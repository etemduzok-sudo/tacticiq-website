...
export const MatchListScreen: React.FC<MatchListScreenProps> = ({
  ...
  const liveMatchRef = useRef<View>(null);
  const firstUpcomingRef = useRef<View>(null);
  ...
  React.useEffect(() => {
    ...
    console.log('🔍 [MatchListScreen] Loading:', loading, 'hasLoadedOnce:', hasLoadedOnce);
  }, [favoriteTeams, pastMatches, liveMatches, upcomingMatches, loading, hasLoadedOnce]);
  ...
  // If we have data already, show it even while loading
  const hasAnyMatches = pastMatches.length > 0 || liveMatches.length > 0 || upcomingMatches.length > 0;
  
  if (loading && !hasLoadedOnce && !hasAnyMatches) {
    return (
