...
export function MatchSquad({ matchData, matchId, lineups, favoriteTeamIds = [], predictionTeamId, onComplete, onAttackFormationChangeConfirmed, isVisible = true, isMatchFinished = false, isMatchLive: isMatchLiveProp, onHasUnsavedChanges }: MatchSquadProps) {
  ...
  // ✅ İlk 11 popup kapanınca sahaya yerleştir
  const handleStartingXIPopupClose = React.useCallback(() => {
    if (!autoGeneratedSquad || startingXIPlayers.length === 0) {
    ...
    // Pozisyon mapping oluştur (GK, RB, CB1, CB2, LB, CM1, CM2, CM3, LW, ST, RW)
    ...
    // Formasyon pozisyonlarını map'le
    attackForm.positions.forEach((pos: string, slotIdx: number) => {
      ...
      else if (normalized.includes('CM') || normalized.includes('CDM') || normalized.includes('CAM')) {
    ...
    const attackPlayersMap: Record<number, typeof players[0] | null> = {};
    const defensePlayersMap: Record<number, typeof players[0] | null> = {};
    
    autoGeneratedSquad.positions.forEach((pos) => {
      const player = startingXIPlayers.find((p: any) => p.player?.id === pos.player.id);
      if (!player) return;
      
      const slots = positionToSlotMap[pos.position] || [];
      if (slots.length > 0) {
        const slot = slots[0];
      ...
      // Defans formasyonu için de aynı mantık
      const defSlots = positionToSlotMap[pos.position] || [];
      if (defSlots.length > 0) {
        const defSlot = defSlots[0];
        defensePlayersMap[defSlot] = {
          ...player.player,
          rating: player.player?.rating || 75,
          position: pos.position,
        };
    ...
    setShowStartingXIPopup(false);
  }, [autoGeneratedSquad, startingXIPlayers, formations]);
  ...
  React.useEffect(() => {
    ...
    const userTeamId = predictionTeamId || getMockUserTeamId(fixtureId) || (favoriteTeamIds.length > 0 ? favoriteTeamIds[0] : null);
    ...
    if (userTeamId) {
      ...
      if (prefs) {
        ...
        // İlk 11 oyuncuları al (lineups'tan)
  ...
  }, [isMatchLive, stateRestored, hasPrediction, matchId, predictionTeamId, favoriteTeamIds, lineups]);
  ...
  if (!selectedFormation && !(isMatchLive && !hasPrediction && autoGeneratedSquad)) {
    return (
  ...
  return (
    <View style={styles.container}>
      <View style={styles.mainContainer}>
        ...
        {isVisible && isMatchLive && !hasPrediction && userPreferenceStats && autoGeneratedSquad && (
            ...
            {isKadroLocked ? (
              <>
                <View style={styles.unifiedToolbarRow}>
                  {/* ✅ Kaynak seçimi - Sadece tahmin yapan kullanıcılar için "Tahminin" göster */}
                  <View style={styles.viewSourceRow}>
                    ...
                    {/* ✅ Tahmin yapmayan kullanıcılar için "Topluluk Tahmini" göster */}
                ...
                <Text style={styles.changeFormationText} numberOfLines={1}>
                  {editingMode === 'attack' 
                    ? `Atak: ${attackFormation || '-'}` 
                    : `Defans: ${defenseFormation || '-'}`}
                </Text>
              </>
            ) : (
      ...
      <Modal
        ...
        animationType="slide"
        transparent={true}
        onRequestClose={() => setShowFormationDetailPopup(null)}
      >
          ...
          <View style={styles.formationDetailModal}>
            ...
            >
              ...
              <View style={styles.formationDetailHeader}>
                ...
                <TouchableOpacity onPress={() => setShowFormationDetailPopup(null)}>
                  <Ionicons name="close-circle" size={24} color="#9CA3AF" />
                </TouchableOpacity>
              </View>
